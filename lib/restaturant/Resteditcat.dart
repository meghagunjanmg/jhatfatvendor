...
class RestEditCategory extends StatelessWidget {

  @override
  Widget build(BuildContext context) {
    Map<String, dynamic> dataLis = ModalRoute.of(context).settings.arguments;
    CategoryRestList Category = dataLis['selectedItem'];
...
class AddState extends State<Add> {
  ...
  void initState() {
    getCategoryList();
    setState(() {
      category_name.text = category.cat_name.toString();
    });
    super.initState();
  }
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          Container(
                            ...
                            child: _image != null
                                ? Image.file(_image)
                                : Image.network(imageBaseUrl+category.cat_image),
                          ),
                          SizedBox(width: 30.0),
                          Icon(
                            Icons.camera_alt,
  ...
  void newHitService(ProgressDialog pr, BuildContext context) async {
    ...
    var request = http.MultipartRequest("POST", Uri.parse(rest_editcategory));
    request.fields["vendor_id"] = '${storeId}';
    request.fields["category_id"] = category.resturant_cat_id.toString();
    request.fields["category_name"] = '${category_name.text}';
    if(_image!=null) {
      request.files.add(
